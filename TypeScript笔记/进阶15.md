1. 类型检查指令
   1. 在前端世界的许多工具中，其实都提供了 行内注释（Inline Comments） 的能力，用于支持在某一处特定代码使用特殊的配置来覆盖掉全局配置。最常见的即是 ESLint 与 Prettier 提供的禁用检查能力，如 /* eslint-disable-next-lint */、<!-- prettier-ignore --> 等。TypeScript 中同样提供了数个行内注释（这里我们称为类型指令），来进行单行代码或单文件级别的配置能力。这些指令均以 // @ts- 开头 ，我们依次来介绍
...
7. 命名空间
   ...
   3. 除了在 .ts 文件中使用以外，命名空间也可以在声明文件中使用，即 declare namespace：
      ...
      5. 除了这两处 namespace 使用，React 中还利用 namespace 合并的特性，在全局的命名空间中注入了一些类型：
        ```
        declare global {
          namespace JSX {
